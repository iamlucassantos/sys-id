...
w_k = np.diagflat(std_w) * np.matrix(np.random.normal(0, 1, [n, N])) + np.diagflat(
    E_w
) * np.matrix(
    np.ones([n, N])
)  # system noise
...
v_k = np.diagflat(std_v) * np.matrix(np.random.normal(0, 1, [nm, N])) + np.diagflat(
    E_v
) * np.ones(
    [nm, N]
)  # measurement noise
...
print(
    "Multi-state IEKF state estimation error RMS = %.3E, completed run with %d samples in %.2f seconds."
    % (np.sqrt((np.square(EstErr_x)).mean()), N, t1 - t0)
)
...
for state in range(0, 2):
    ...
    ax[state].set_title("state" + str(state + 1))
    ax[state].legend(
        ["estimation error", "upper error STD", "lower error STD"], loc="upper right"
    )
plt.show()
if printfigs == True:
